...
class ImageHandler:
    ...
    async def process_message_images(
        ...
        try:
            ...
            # 检查消息中是否有图片
            has_image, has_text, image_components = ImageHandler._analyze_message(message_chain)
            
            # 如果没有图片,直接返回原消息
            if not has_image:
                return True, event.get_message_outline()
            
            logger.debug(f"检测到消息包含 {len(image_components)} 张图片, 是否有文字: {has_text}")
    ...
    @staticmethod
    def _analyze_message(message_chain: List[BaseMessageComponent]) -> Tuple[bool, bool, List[Image]]:
        """
        分析消息链，检查图片和文字
        ...
        try:
            ...
            for idx, img_component in enumerate(image_components):
                try:
                    ...
                    # 使用用户配置的超时时间
                    description = await asyncio.wait_for(call_vision_ai(), timeout=timeout)
                    ...
                    logger.warning(f"图片 {idx} 转文字超时（超过 {timeout} 秒），可在配置中调整 image_to_text_timeout 参数")
                    continue
                except Exception as e:
