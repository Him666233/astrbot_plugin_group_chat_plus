...
class ContextManager:
    ...
    def init():
        """初始化上下文管理器，创建存储目录"""
        ContextManager.base_storage_path = os.path.join(os.getcwd(), "data", "chat_history")
    ...
    def _get_storage_path(platform_name: str, is_private: bool, chat_id: str) -> str:
        ...
        directory = os.path.join(ContextManager.base_storage_path, platform_name, chat_type)
    ...
    def get_history_messages(
        ...
        try:
            ...
            # 读取历史消息文件
            file_path = ContextManager._get_storage_path(platform_name, is_private, chat_id)
        ...
        try:
            ...
            if history_messages:
                ...
                for msg in history_messages:
                    ...
                    if is_bot:
                        # AI自己的回复,特殊标注
                        formatted_parts.append(f"[{time_str}] 你自己回复: {message_content}")
                    else:
                        # 其他用户的消息
                        formatted_parts.append(f"[{time_str}] {sender_name}(ID:{sender_id}): {message_content}")
    ...
    async def save_user_message(
        ...
        try:
            ...
            user_msg.type = MessageType.GROUP_MESSAGE if not is_private else MessageType.FRIEND_MESSAGE
            ...
            user_msg.session_id = event.session_id if hasattr(event, 'session_id') else chat_id
            ...
            if context:
                try:
                    ...
                    logger.debug("用户消息已保存到官方历史管理器(platform_message_history)")
                    
                except Exception as e:
                    logger.warning(f"保存到官方历史管理器(platform_message_history)失败: {e}")
                    # 即使官方保存失败，自定义存储仍然成功
    ...
    async def save_bot_message(
        ...
        try:
            ...
            bot_msg.type = MessageType.GROUP_MESSAGE if not is_private else MessageType.FRIEND_MESSAGE
            ...
            bot_msg.session_id = event.session_id if hasattr(event, 'session_id') else chat_id
            ...
            if context:
                try:
                    ...
                    logger.debug("AI回复消息已保存到官方历史管理器(platform_message_history)")
    ...
    async def save_to_official_conversation(
        ...
        try:
            ...
            logger.debug(f"[官方保存] 准备保存到官方对话系统，会话: {unified_msg_origin}")
            ...
            if not curr_cid:
                logger.info(f"[官方保存] 会话 {unified_msg_origin} 没有对话，创建新对话")
                # 获取群名作为标题
                chat_id = event.get_group_id() if not event.is_private_chat() else event.get_sender_id()
                title = f"群聊 {chat_id}" if not event.is_private_chat() else f"私聊 {event.get_sender_name()}"
            ...
            logger.debug(f"[官方保存] 准备保存，新增2条消息，总计 {len(history_list)} 条")
            ...
            if success:
                logger.info(f"✅ [官方保存] 消息已保存到官方对话系统 (conversation_id: {curr_cid}, 总消息数: {len(history_list)})")
                return True
            else:
    ...
    @staticmethod
    async def _try_official_save(cm, unified_msg_origin: str, conversation_id: str, history_list: list) -> bool:
        """
        尝试多种方法保存到官方对话管理器
        ...
        try:
            ...
            try:
                ...
                logger.info(f"[官方保存] CM类型={cm_type}, 对话ID={conversation_id}, 消息数={len(history_list)}")
            ...
            for m in methods:
                if hasattr(cm, m):
                    # 尝试位置参数+列表
                    try:
                        logger.info(f"[官方保存] >>> 尝试 {m} 使用列表参数，历史长度={len(history_list)}")
                        await getattr(cm, m)(unified_msg_origin, conversation_id, history_list)
                        logger.info(f"✅ [官方保存] {m} 成功（列表）")
                        
                        # 验证是否真的保存成功
                        try:
                            verification = await cm.get_conversation(unified_msg_origin, conversation_id)
                            if verification:
                                logger.info(f"✅ [官方保存] 验证成功：对话存在，ID={conversation_id}")
                            else:
                                logger.warning(f"[官方保存] 验证失败：无法获取刚保存的对话")
                    ...
                    try:
                        ...
                        logger.info(f"[官方保存] >>> 尝试 {m} 使用字符串参数，长度={len(history_str)}")
            ...
            logger.error(f"❌ [官方保存] 所有保存方法均失败！消息可能未保存到官方系统！")
            return False
    ...
    async def save_to_official_conversation_with_cache(
        ...
        try:
            ...
            # 2. 获取conversation_manager
            cm = context.conversation_manager
            logger.info(f"[官方保存+缓存转正] ConversationManager类型: {type(cm).__name__}")
            ...
            if not curr_cid:
                logger.info(f"[官方保存+缓存转正] ❗ 会话 {unified_msg_origin} 没有对话，准备创建新对话")
                # 获取群名作为标题
                chat_id = event.get_group_id() if not event.is_private_chat() else event.get_sender_id()
                title = f"群聊 {chat_id}" if not event.is_private_chat() else f"私聊 {event.get_sender_name()}"
                ...
                try:
                    ...
                    logger.info(f"✅ [官方保存+缓存转正] 成功创建新对话，ID: {curr_cid}")
                except Exception as create_err:
                    logger.error(f"❌ [官方保存+缓存转正] 创建对话失败: {create_err}", exc_info=True)
                    return False
            ...
            try:
                ...
                logger.info(f"[官方保存+缓存转正] 获取对话对象: {conversation is not None}")
                if conversation:
                    logger.info(f"[官方保存+缓存转正] 对话对象类型: {type(conversation).__name__}")
                    logger.info(f"[官方保存+缓存转正] 对话标题: {getattr(conversation, 'title', 'N/A')}")
            except Exception as get_err:
                logger.error(f"❌ [官方保存+缓存转正] 获取对话失败: {get_err}", exc_info=True)
                conversation = None
            ...
            if conversation and conversation.history:
                ...
                try:
                    ...
                    logger.info(f"[官方保存+缓存转正] 解析历史记录成功: {len(history_list)} 条")
            ...
            if cached_messages:
                ...
                logger.info(f"[官方保存+缓存转正] 现有历史内容数: {len(existing_contents)} 条")
                ...
                for cached_msg in cached_messages:
                    if isinstance(cached_msg, dict) and 'content' in cached_msg:
                        if cached_msg['content'] not in existing_contents:
                            ...
                            existing_contents.add(cached_msg['content'])  # 避免缓存内部重复
                            added_count += 1
                        else:
                            skipped_count += 1
                
                logger.info(f"[官方保存+缓存转正] 缓存消息处理完成: 总数={len(cached_messages)}, 添加={added_count}, 跳过(重复)={skipped_count}")
            else:
            ...
            logger.info(f"[官方保存+缓存转正] 准备保存，总消息数: {len(history_list)} 条")
            ...
            if success:
                # 计算实际转正的缓存数量
                cache_converted = len([m for m in cached_messages if isinstance(m, dict) and 'content' in m])
                logger.info(f"=" * 60)
            ...
            logger.error(f"❌❌❌ [官方保存+缓存转正] 保存过程发生严重异常: {e}", exc_info=True)
            return False
