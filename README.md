...
- **👆 戳一戳支持** - 智能处理戳一戳消息，AI能识别并回应（v1.0.9新增）
- **🚫 @他人过滤** - 可配置忽略@其他人的消息，避免插入他人对话（v1.0.9新增）
- **🔒 戳一戳防伪造** - 自动过滤手动输入的戳一戳文本标识符，防止误判（v1.0.9增强）
...
| `poke_bot_skip_probability` | bool | true | **戳机器人时跳过概率筛选**<br>仍保留AI读空气判断 |
...
缓存消息(含mention_info+trigger_type+poke_info，系统提示临时用于AI理解) → 提取历史上下文 ← 合并缓存消息 → 智能去重
    ↓
AI决策判断(可见系统提示，理解@标记+戳一戳提示+防重复) → 注入情绪状态 → 注入记忆 → 注入工具信息
    ↓
