...
> **图片处理须知：** 两种模式可选——(1) 配置 `image_to_text_provider_id` 后图片会被 AI 转写为 `[图片内容: ...]` 再参与上下文，结果自动缓存省钱（**推荐**）；缓存命中优先，剩余图片再逐张调用视觉模型，失败或超时时会在原位置留下清晰的 `[图片（识别失败）]` 占位，不传递原始图片；(2) 留空 `image_to_text_provider_id` 时直接传递原始图片给多模态 AI（需使用支持图片的多模态模型），文本中内联 `[图片: /path/to/image.png]` 标记（与视频/语音/文件格式一致），且不受 `max_images_per_message` 截断；若某张图片路径提取失败，该位置会降级为 `[图片（识别失败）]`。直接发送的图片和引用消息组件链中的图片使用同一套处理、缓存与数量限制，并保留图片所在的引用上下文。若关闭 `enable_image_processing` 或当前消息不在处理范围内，插件会直接剥离原始图片；过滤后严格等于空字符串 `""` 的消息会被丢弃，只剩空格/换行等空白字符仍视为有内容。未回复缓存与窗口缓存属于被动历史收集，只保留已解析的图片信息（`[图片内容: ...]`、`[Image: xxx]`、`[图片: /path/to/image.png]`），未解析图片占位（`[图片]`、`[Image]`、`[图片（识别失败）]`）会被移除。视频、语音、文件等非图片媒体会被插件自动提取并内联到消息文本中（格式如 `[视频: /path]`、`[语音: /path]`、`[文件: name, /path]`），AI 可通过工具调用自行解析处理。
