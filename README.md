...
> **建议开启平台的"群聊上下文感知"！** 旧版 AstrBot（≤4.25.1）需要开启它，平台才会稳定写入本插件优先读取的官方群聊历史；新版 AstrBot（≥4.25.2）本插件已兼容官方会话读取与自定义存储回退，不开启也不会直接导致主链路不可用，但开启后历史、图片描述与第三方插件协作更完整。推荐配置与原因说明见：[深度指南 → 平台配置](docs/ARCHITECTURE.md#推荐的平台设置)
>
> **AstrBot 4.25.2 兼容性说明**：从 4.25.2 起，平台内部的"群聊上下文感知"模块从 `LongTermMemory`（`session_chats`，注入到 `system_prompt`/`prompt`）重构为 `GroupChatContext`（`raw_records`，注入到 `extra_user_content_parts`）。本插件已完整适配——图片描述提取自动兼容新旧两套存储，重复群聊历史只在严格识别为平台 GCC 块时移除（精确文案/结构特征 + `<system_reminder>` 框架 + 平台记录行），识别失败时优先保留，第三方插件注入不受影响。同一份配置在 ≤4.25.1 和 ≥4.25.2 上均可正常工作，无需手动切换。详见：[深度指南 → AstrBot 4.25.2 变更详解](docs/ARCHITECTURE.md#astrbot-4252-变更longtermmemory--groupchatcontext)

> **图片处理须知：** 两种模式可选——(1) 配置 `image_to_text_provider_id` 后图片会被 AI 转写为 `[图片内容: ...]` 再参与上下文，结果自动缓存省钱（**推荐**）；缓存命中优先，剩余图片再逐张调用视觉模型，失败或超时时会在原位置留下清晰的 `[图片（识别失败）]` 占位，不传递原始图片；(2) 留空 `image_to_text_provider_id` 时直接传递原始图片给多模态 AI（需使用支持图片的多模态模型），文本中内联 `[图片: /path/to/image.png]` 标记（与视频/语音/文件格式一致），且不受 `max_images_per_message` 截断；若某张图片路径提取失败，该位置会降级为 `[图片（识别失败）]`。若关闭 `enable_image_processing` 或当前消息不在处理范围内，插件会直接剥离原始图片；过滤后严格等于空字符串 `""` 的消息会被丢弃，只剩空格/换行等空白字符仍视为有内容。未回复缓存与窗口缓存属于被动历史收集，只保留已解析的图片信息（`[图片内容: ...]`、`[Image: xxx]`、`[图片: /path/to/image.png]`），未解析图片占位（`[图片]`、`[Image]`、`[图片（识别失败）]`）会被移除。视频、语音、文件等非图片媒体会被插件自动提取并内联到消息文本中（格式如 `[视频: /path]`、`[语音: /path]`、`[文件: name, /path]`），AI 可通过工具调用自行解析处理。
...
> **Web 面板访问安全说明**：除登录页、登录所需静态资源、Logo、认证状态探测和 `robots.txt` 外，所有面板页面、面板静态资源和 `/api/*` 非公开接口都会在后端统一经过 IP 黑白名单/封禁检查、JWT + 服务端会话校验，以及防爬虫/已登录速率限制。`robots.txt` 对所有来源可读并声明 `Disallow: /`，用于让正常爬虫先看到禁止抓取规则；统一错误页额外带有禁止索引标记，避免拦截页被搜索引擎收录。
...
| **向 `extra_user_content_parts` 追加内容** | 插件通过新版 AstrBot API 在用户消息之外附加内容块 | 原样保留在 `extra_user_content_parts` 中；仅严格匹配平台 GCC 群聊历史块时移除，识别失败则保留避免误删 |
...
- **工具调用记录保存长度可配置** — 新增 `tool_call_max_args_length`（默认5000字符）和 `tool_call_max_result_length`（默认10000字符）配置项，控制单个工具调用的参数和结果在历史记录中的保存长度。支持三种模式：完全省略（0，使用占位符）、自定义截断（正数，超出部分截断并添加"..."）、完全不限制（-1，真正无上限）。某些工具会在调用参数中包含完整的回复内容，提升默认限制后可避免重要信息被截断影响 AI 上下文理解
...
- **速率窗口机制加固** — 自动刷新请求会携带 `X-GCP-Auto-Refresh` 标记，但该标记只用于前端区分请求来源，不是后端豁免依据；数据图表、会话管理、心跳状态展示等自动刷新请求仍计入已登录速率窗口，防止伪造请求头绕过限速。只有专用心跳接口 `/api/auth/heartbeat` 不计入已登录速率窗口，但仍必须通过 IP 可访问性、JWT 真实有效和服务端会话校验
...
- **AI读空气** — 两层过滤：概率筛选 + AI智能判断，精准控制回复时机；在 Smart 并发模式下，读空气判断会基于合并批次视图（含 Smart 吸收的追加消息与同群相关 GWW 窗口的缓冲消息），并在开启 Smart 批次提示增强时收到专门的批次感知提示，明确当前消息是判断主对象、追加消息仅作背景参考
...
- **智能缓存** — "缓存+转正"机制，未回复消息保留上下文，下次回复时自动合并；支持冷群自动转正，群聊长时间静默且会话空闲后自动将普通缓存与遗留窗口缓存写入历史，避免过期丢失
- **记忆系统** — 支持 LivingMemory（混合检索+人格隔离）和 Legacy 双模式，auto 模式自动检测适配插件
- **并发协调** — 群聊支持 legacy / smart 两种并发模式；smart 会按真实到达顺序选主消息并批量感知追加消息，追加消息先保存在 Smart 专用批次快照中，不进入普通缓存池或 GWW 窗口缓存池。给读空气 AI 和回复 AI 展示时，Smart 复用“当前消息后紧接着又收到的消息”这套上下文展示格式；该展示只看调用方传入的追加消息列表，不要求开启 GWW。Smart 与 GWW 协同开启时，若同一 Smart 批次涉及多个窗口，才会为这些窗口建立逻辑融合组，让读空气 AI、回复 AI、追加消息标记、窗口回落与 Phase-2 保存都按同一批窗口消息处理；普通未融合窗口仍按原本 `sender_id + gww_token` 精确隔离逻辑运行。Smart 吸收的追加消息会保留各自的 @ 解析、@全体说明、戳一戳事件、发送者与环境元数据；纯戳一戳这类平台侧空正文组件会以 `[ComponentType.Poke]` 占位保存。legacy 保持传统串行兜底；普通对话、主动对话、冷群转正之间按会话 owner 自动互斥，等待超时会保守回退为缓存，无需额外配置
- **Smart批次提示增强** — 可选开关（`enable_smart_batch_reply_hint`，默认开启）。开启后，Smart 模式下会分别在决策阶段和回复阶段动态插入提示：决策阶段提示读空气 AI 当前消息是判断主对象、追加消息仅作背景参考；回复阶段提示回复 AI 当前触发 anchor 消息的用户仍是主要回复对象，但可以像真人一样自然顺带回应批次中来自其他用户的消息，不值得回的消息也可以大方忽略。这些提示只存在于运行时上下文，保存历史前会自动过滤（通过 `MessageCleaner` / `ContextManager` 清洗），不会污染长期上下文
...
- **多媒体消息** — 支持图片（转文字或直传多模态）、视频、语音、文件等媒体类型的识别与传递；媒体文件路径内联到消息文本中（如 `[视频: /path]`、`[语音: /path]`、`[文件: name, /path]`），AI 可通过 Agent 工具调用自行解析；缓存时会清空不可长期依赖的图片路径，原始 `[图片]` / `[Image]` 与图片识别失败占位会被移除，成功解析的 `[图片内容: ...]` 以及视频/语音/文件标记会保留
- **转发解析** — 面向 QQ / OneBot 场景解析合并转发消息，支持在深度限制内展开嵌套转发，并将整条转发内容折叠为单条可读文本继续参与后续 AI 流程。转发内部的文本、引用、@、图片、视频、语音、文件等消息段会按原顺序逐段保留；媒体可识别时输出 `[图片: ...]`、`[视频: ...]`、`[语音: ...]`、`[文件: ...]` 等原始信息标记，无法识别时仅在对应位置降级为 `[图片（识别失败）]`、`[视频（识别失败）]`、`[语音（识别失败）]`、`[文件（识别失败）]`。引用消息和嵌套转发会继续递归使用同一套解析与局部占位逻辑；表情包图片仅在平台返回 `sub_type`、`summary`、`image_type` 等可识别字段时追加 `[表情包图片]` 标记
- **关键词系统** — 触发词跳过概率/智能模式，黑名单词直接过滤
- **戳一戳** — 智能响应QQ戳一戳，支持反戳和回复后戳。戳一戳事件解析为两层：**`[戳一戳事件]`**（持久化事件文本，注入到消息格式中「冒号前」的系统元数据区域，随消息一起保存到历史上下文，不会被后续过滤清理）与 **`[戳一戳提示]`**（运行时提示，追加在上下文分隔符 `=====` 之外，仅当前轮 AI 可见，保存时由 `MessageCleaner` 过滤规则自动移除）。对本插件**实际接手处理**的真实戳一戳事件，会把"谁戳了谁"的事件语义保留到历史上下文中（官方存储与自定义存储双写），便于后续AI理解；平台侧仅提供 `Poke` 组件且正文为空时，会用 `[ComponentType.Poke]` 作为用户内容占位，避免历史中只剩元数据。戳一戳消息不会被 GWW 等待窗口缓存吸收，但可以在 Smart 批次中作为追加消息被统一感知和保存。触发反戳时，会先后保存用户戳一戳事件（user视角）和AI反戳动作（assistant视角），均绑定当前会话。启用"戳过对方追踪提示"后，还会在短时间内追踪被AI戳过的用户；若追踪人数超过上限，会移除当前最早登记的记录。此行为无独立配置项，是否生效取决于当前 `poke_message_mode`、平台是否为 QQ+aiocqhttp/OneBot poke notice，以及该群是否允许本插件处理戳一戳消息
- **群身份标签** — 自动检测群成员的QQ群身份（群主/管理员/群成员），在发送者信息、AI身份提示、@消息标签、AI回复保存前缀等位置显示角色标签。需启用 `enable_group_role_display` 开关，各注入位置分别依赖对应功能开关（如「包含发送者信息」、「启用AI身份提示」等）。仅支持QQ平台(aiocqhttp)，其他平台自动跳过。角色信息缓存5分钟避免重复API调用
- **@消息优先** — @机器人消息跳过所有判断直接回复；重复 @AI 会按同一发送者的空 @ / 普通 @ 规则处理，不会误判成 @他人。`@全体成员` 与 `@他人过滤` 独立处理，可单独配置为按普通消息处理、跳过概率筛选、跳过概率+读空气，或仅对当前这条消息临时提升概率；`@AI + @他人` 的混合消息会保留完整 @ 解析并添加指向说明，窗口吸收时也只会剥离 AI 自己的 At，不会移除或折叠其他人的 At。对于单独的、不包含任何信息的 @ 消息，系统会默认启用中性上下文强化，并在通过前置过滤与读空气筛选后，再提醒 AI 优先参考最近上下文但不要强行续话；这层关联会同时受时间窗口和消息数窗口约束。`@全体成员` 的解析说明会随消息一起保存，供后续缓存转正与历史上下文继续使用；若插件已决定将这类唤醒消息忽略、窗口吸收或仅缓存不回复，会只消费当前事件的 AstrBot 默认 LLM 兜底，避免平台在插件接管后再次强制回复，同时不主动阻断其他插件处理
- **引用/回复消息** — 自动解析消息中的引用（Reply）组件，格式为 `[引用 >>> 发送者(ID): 被引用的消息内容]`，使用 `>>>` 符号明确分隔"引用"标记与实际的引用内容，帮助 AI 清晰区分哪部分是引用的消息、哪部分是用户正文。引用内容末尾自动追加换行，使正文另起一行，视觉上不会有歧义。若被引用消息的发送者为 AI 自身，会在发送者名称后标注 `(你)`，帮助 AI 识别自己被引用的消息；默认还会将其视为直接触发，跳过概率筛选。该行为可通过 `reply_bot_skip_probability` 关闭，引用其他用户不受影响。当被引用内容无法提取时，若发送者信息可用，保留引用框架并标注 `(无法获取引用内容)`；仅当发送者和内容全部缺失时才跳过
...
>
> **Web 面板会话补充说明**：当前版本的 Web 面板会话已升级为 `JWT + HttpOnly Cookie + 服务端会话表`。登录页遇到有效会话时会直接跳转到面板；若检测到令牌过期、密码已修改、服务端重启或（开启 `web_panel_ip_bind_check` 时）IP 变化，会统一要求重新登录。前台/后台心跳频率、失败重试基准与最大重试间隔现已提供独立配置项，但这些参数属于安全敏感配置，只能通过 AstrBot 传统配置界面修改，Web 面板中为只读显示。同一浏览器多标签页只会选出一个 Leader 标签页发送认证心跳，其他标签页监听状态广播，避免重复探测。
>
> **Web 面板接口安全说明**：直接绕过前端调用后端接口不会绕过校验。除登录、认证状态探测、Logo 和 `robots.txt` 等公开入口外，所有 `/api/*` 非公开接口都必须同时满足当前 IP 可访问、令牌真实存在且未过期、服务端会话有效，才会返回数据或执行写入操作。令牌缺失、无效或过期时，受保护 API 会在中间件层返回 401，不会执行对应业务处理；浏览器地址栏直接访问 API 会在完成安全检查后跳转到统一无权限错误页，不返回业务数据。登录、改密、配置、封禁、文件管理、分页等输入均在后端校验类型、长度和可接受范围；历史访问日志和封禁备注返回前会做文本清洗。前端校验只用于交互提示，不作为安全边界。
>
> **Web 面板自动刷新与限速说明**：数据图表、会话管理、心跳状态展示等自动刷新请求会计入已登录请求速率窗口，防止攻击者伪造自动刷新请求头绕过限速。前端会通过防重入、慢请求不叠加、后台标签页暂停展示类刷新等方式降低正常使用误触发限速的概率。只有 `/api/auth/heartbeat` 专用认证心跳不计入已登录速率窗口，但它仍必须通过当前 IP 可访问和令牌真实有效校验。
>
> **Web 面板文件/会话/缓存边界说明**：当前 Web 面板中的文件管理仅允许操作插件数据目录下的普通数据文件；`auth.json`、`jwt_secret.json`、`sessions.json`、访问日志、封禁数据及其同名备份文件等核心安全文件不会在文件列表中暴露，读取、修改和删除也都会被后端直接拒绝。会话管理中的聊天记录查看/编辑针对的是插件自定义 `chat_history/...` 历史文件，不是 AstrBot 官方 ConversationManager 历史。图片缓存相关功能则只作用于图片描述缓存文件 `image_cache/descriptions.jsonl`（若检测到旧版残留路径 `image_description_cache.json` 也会兼容处理），不会清理聊天记录、注意力、概率或主动对话状态。
>
> **反向代理补充说明**：如果反向代理与 Web 面板部署在同一台机器，系统在检测到连接来源为 `127.0.0.1 / ::1` 时会自动读取合法的 `X-Real-IP / X-Forwarded-For` 获取真实客户端 IP，因此即使未开启 `web_panel_trust_proxy`，也可能正常拿到真实 IP；若反向代理不在本机，则需要显式开启 `web_panel_trust_proxy` 才会信任代理头。该项现已归类为安全边界配置，只能在传统配置界面修改。
>
...
### 关于历史消息清除（platform_message_history 与 conversations）

AstrBot 有两张独立的存储表：

| 表名 | 存储内容 | `/reset` 是否清理 | 插件截止过滤是否生效 |
|---|---|---|---|
| `platform_message_history` | 原始平台消息记录 | ❌ 不清理 | ✅ 生效（基于 `created_at` 时间戳） |
| `conversations` | LLM 对话上下文（OpenAI 格式） | ✅ 清空为 `[]` | ❌ 不生效（消息无时间戳字段，过滤无效） |

AstrBot 的 `/reset` 指令只清除 `conversations` 表，**不会**清除 `platform_message_history` 表。反之，本插件的截止时间戳机制**只过滤 `platform_message_history` 的读取**，不会清除 `conversations` 表。

**本插件的解决方案**：执行 `gcp_reset` 或 `gcp_reset_here` 指令后，插件会记录一个截止时间戳。此后从 `platform_message_history` 表读取历史消息时，截止点之前的所有消息都会被自动过滤——表里的数据虽然还在，但 AI 从这一路看不到。

**⚠️ 重要：如何彻底清空当前会话的全部历史**

由于两张表分别存储不同类型的数据，**仅执行插件重置指令无法完全清除 AI 上下文**。如需彻底清空当前会话的所有历史记忆，请**依次执行**：

1. **`gcp_reset_here`**（或 Web 端对应按钮）— 清理插件运行态 + 设置 `platform_message_history` 截止过滤 + 删除本地聊天记录文件
2. **`/reset`**（平台指令）— 清空 `conversations` 表（LLM 对话上下文）

两步缺一不可。只做 1 不做 2，AI 仍会从 `conversations` 表读取旧的 LLM 对话上下文；只做 2 不做 1，`platform_message_history` 中的旧消息仍可被读取。

**边界说明**：
- `gcp_reset` = **全局重置**，清理插件维护的全局运行态与本地持久化缓存（如自定义 `chat_history` 历史目录、注意力/主动对话持久化文件等），并为所有已知会话设置历史截止时间戳
- `gcp_reset_here` = **单会话重置**，完整清理当前会话的所有插件运行态（与命令行完全相同的 20+ 项清理）、当前会话对应的自定义聊天记录文件，并设置历史截止时间戳。**Web 端「指令快捷操作→会话重置」与命令行版本使用完全相同的清理逻辑**
...
执行 `gcp_reset_here` 后，插件记录截止时间戳，之后 AI 不再读取 `platform_message_history` 中截止点之前的旧消息，无需操作数据库。若还需清除 LLM 对话上下文，再执行平台的 `/reset`。
...
> - `image_to_text_provider_id` 填写后启用图片转文字；留空则走多模态直传模式（需要回复模型支持图片）
> - `decision_ai_provider_id` 留空使用默认提供商，建议使用轻量快速的模型。如需冗余保障，可配置 `decision_ai_fallback_provider_ids`（判断型AI三处共用）、`image_to_text_fallback_provider_ids`（图片转文字）、`proactive_gen_fallback_provider_ids`（主动对话生成）作为备用提供商列表；调用超时/报错/空响应时自动按顺序切换，不留空即不启用
...
> - `smart` 与 GWW 同时开启时，如果同一 Smart 批次涉及多个等待窗口，插件会为这些窗口打逻辑融合标记。后续读取窗口上下文、读空气判断、回复生成、AI 回复追加消息标记、判断不回复后的窗口回落，以及 Phase-2 保存/清理都会按该融合组统一处理；未参与融合的普通窗口不受影响
...
追加消息会复用"当前消息后紧接着又收到的消息"这套上下文表达，保留发送者名字、ID 与时间信息（若相关配置开启），让 AI 自己判断这些消息是否需要一并参考。这里复用的是展示格式，不等于 Smart follower 写入了 GWW 窗口缓存；Smart 自身通过专用批次快照保存完整 `cached_data`，窗口开关关闭时仍会展示给读空气 AI 和回复 AI。Smart 吸收的追加消息会保留各自的 @ 解析、@全体说明、戳一戳事件、图片/媒体/转发/引用等解析结果与环境元数据；平台侧空正文的真实 Poke 组件会用 `[ComponentType.Poke]` 作为内容占位，避免保存时只剩元数据。
...
- 被 GWW 拦截吸收的消息不会再进入 Smart claim_batch 批处理流程
- Smart 自己的批次快照不属于普通缓存或 GWW 窗口缓存；只有异常回退、读空气判定不回复、并发 owner 占用等兜底路径才会显式转为普通缓存
- 追加消息展示区是通用上下文格式，只要 Smart 或 GWW 调用方传入消息列表就会展示；GWW 开关只影响是否额外收集窗口缓存，不影响 Smart 自身追加消息展示
- **Smart + GWW 窗口融合**：在 Smart 模式下若同时开启了 GWW，同一 Smart 批次涉及多个等待窗口时，插件会为这些窗口打运行时融合标记。AI 的上下文视野会按融合组读取窗口缓冲消息；判断不回复后的回落、Phase-2 保存与清理也按同一融合组处理。未参与融合的普通窗口仍按原有令牌独立处理，不受影响
...
回落后的消息与普通缓存完全一致——按时间戳参与上下文排序、Phase-1 转正、冷群自动转正，确保下次回复时上下文顺序始终正确，旧窗口消息不会再以"追加消息"形式误拼入新对话。Smart 批次回落与 GWW 窗口回落各自处理不同的消息来源，互不干扰；如果当前 Smart 批次存在窗口融合组，GWW 窗口回落会按融合组一次性转换该组内全部窗口缓冲消息，并在转换为普通缓存时移除运行时融合标记。

### 主动对话与普通对话的协调

主动对话、普通对话、冷群转正之间现在会自动做会话级互斥：

- **已开始的链路先完成**：普通回复、主动对话、冷群转正都会登记当前会话 owner，其他链路先等待 owner 释放
- **普通消息不强行抢跑**：如果主动对话或冷群转正已经占用会话，新普通消息会先等待；等待超时后回退为普通缓存，避免在未落库的主动回复旁边生成错位回复
- **主动对话自动避让**：主动对话开始前会检查当前群聊是否已有普通对话或其他 owner 在处理
- **释放后重新判断**：主动对话若等待普通链路释放，释放后会重新跑一次触发判断；不满足条件则跳过本轮，等待下一个定时周期重新收集上下文和预判断
- **冷群转正只在完全空闲时执行**：会等待普通处理、主动对话、Smart pending 和已有 owner；保存失败或占用冲突时只跳过本轮，不清缓存
- 这套协调是**内部强制生效**的，不需要新增任何配置；如果没开启主动对话或冷群转正，相关保护基本不会参与
...
- 只有相关场景真正发生时才会插入
- Smart 批次提示在决策阶段（读空气 AI）和回复阶段分别注入，决策阶段侧重"判断对象是当前消息"，回复阶段侧重"如何组织回复"
...
- 当 `concurrent_mode=smart` 且开启 `enable_smart_batch_reply_hint` 时，决策阶段和回复阶段会分别动态追加 Smart 批次提示：决策阶段提示判断对象是当前消息（追加消息仅作背景）；回复阶段提示当前触发对象仍是主要回复对象，但可以像真人一样自然顺带回应批次中的其他消息
- 这两类提示词和 Smart 批次提示都属于**运行时生成提示词**，职责分别是帮助 AI 做判断和直接产出要发送的话
...
**📦 Agent 调用兼容性全面扩充**:
- **视频/语音/文件三类非图片媒体完整支持** — 在 `image_handler.py` 的媒体提取管线中新增三个独立提取通道：① 视频组件（`Video`）→ 提取文件路径，注入 `[视频: /data/videos/xxx.mp4]` 内联标记（失败时降级为 `[视频]`）；② 语音组件（`Record`）→ 提取音频路径，注入 `[语音: /path]` 标记，路径同时注入 `audio_urls` 字段供 LLM 访问；③ 文件组件（`File`）→ 提取文件名和路径，注入 `[文件: 报告.pdf, /path]` 标记。三类媒体覆盖消息链遍历、标记内联注入、缓存剥离前富化、保存传递全链路。V1.2.3.hotfix.2 起，`video_paths` 和 `file_infos` 同步存入 `cached_message` 字典与 `event extras`（`_plugin_media_video_paths` / `_plugin_media_file_infos`），`on_llm_request` 钩子对其进行与 `image_urls`/`audio_urls` 平行的去重合并，第三方插件可完整读取所有非图片媒体数据
- **纯语音/纯文件/纯视频消息防丢弃** — `image_handler.py` 的 `_extract_image_urls_structured()` 返回值从二元组 `(image_urls, image_descriptions)` 扩展为五元组 `(image_urls, image_descriptions, audio_urls, video_paths, file_infos)`，调用方同步更新。语音/视频/文件消息在 `main.py` 的空消息入口过滤环节被视为有效内容，防止纯多媒体合法消息被误判为真空消息而丢弃。V1.2.3.hotfix.2 起，提取阶段、消息链遍历、上下文回读、第三方插件传递全链路均保留具体媒体类型和文件路径信息
